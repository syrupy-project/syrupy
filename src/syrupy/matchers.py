...
def path_name_str(
    ...
    *,
    ...
    """
    Factory to create a matcher using path and type mapping.
    Behaves similar to `path_type` except that it returns a key of an element
    instead of its type
    """
    if not mapping and not types:
        raise PathTypeError(gettext("Both mapping and types argument cannot be empty"))
    ...
    def _capitalize(path: "PropertyPath") -> str:
        # assuming path is in form (("key", type(key),)
        return ("<" + str(path[-1][1].__name__) + ">").upper()
