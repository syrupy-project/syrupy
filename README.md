...
| `--snapshot-file-lock` | **Experimental.** Use a file lock and atomic replace when writing amber snapshot files so concurrent `pytest-xdist` workers do not clobber each other during `--snapshot-update`. Same-machine workers only; not reliable across remote workers / NFS. | `False` |
...
- `pytest-xdist`: unused snapshot detection and removal are supported when running with multiple processes. Each worker reports the snapshots it used and the controller combines them to determine which snapshots are unused. Concurrent `--snapshot-update` writes to the same multi-entry amber file can still race unless you opt into `--snapshot-file-lock` (same-machine workers only; not reliable across remote workers / NFS). The opt-in uses advisory file locks and atomic replace; a follow-up may make this the default after validation. Independently of that flag, xdist worker reports compress the collected-items payload to reduce IPC size on large suites. See [#535](https://github.com/syrupy-project/syrupy/issues/535) and [#1237](https://github.com/syrupy-project/syrupy/issues/1237) for more information.
