...
| `--snapshot-file-lock-timeout` | Seconds to wait for an exclusive amber write lock when pytest-xdist is distributing work. Locking is enabled automatically under xdist. | `60` |
...
- `pytest-xdist`: unused snapshot detection and removal are supported when running with multiple processes. Each worker reports the snapshots it used and the controller combines them to determine which snapshots are unused. Concurrent `--snapshot-update` writes to the same multi-entry amber file are guarded automatically with advisory file locks and atomic replace (same-machine workers only; not reliable across remote workers / NFS). Lock wait time is configurable via `--snapshot-file-lock-timeout` (default 60s). Independently, xdist worker reports compress the collected-items payload to reduce IPC size on large suites. See [#535](https://github.com/syrupy-project/syrupy/issues/535) and [#1237](https://github.com/syrupy-project/syrupy/issues/1237) for more information.
